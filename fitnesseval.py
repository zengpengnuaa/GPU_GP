...
# fitness evaluating
def fitness_func_2(indi, lock):
    cuda.select_device(2)
    try:
        innerfunc_string = 'lambda ARG0: ' + str(indi)
        file_path = './cuda_func_2.py'
        lock.acquire()
        fd = open(file_path, mode='r')
        content = fd.read()
        pos = content.find('innerfunc')
        ...
        d_imgs = cuda.to_device(oral_data)
        ret_vectors = cuda.device_array((threads_num, vector_dim))
        cuda_func_2.cudafunc[blockspergrid, threadsperblock](d_imgs, ret_vectors)
        results = ret_vectors.copy_to_host()
        cuda.close()
        train_tf = np.asarray(results)
        min_max_scaler = preprocessing.MinMaxScaler()
...
def fitness_func_3(indi, lock):
    cuda.select_device(0)
    try:
        innerfunc_string = 'lambda ARG0: ' + str(indi)
        file_path = './cuda_func_3.py'
        lock.acquire()
        fd = open(file_path, mode='r')
        content = fd.read()
        pos = content.find('innerfunc')
        ...
        d_imgs = cuda.to_device(oral_data)
        ret_vectors = cuda.device_array((threads_num, vector_dim))
        cuda_func_3.cudafunc[blockspergrid, threadsperblock](d_imgs, ret_vectors)
        results = ret_vectors.copy_to_host()
        cuda.close()
        train_tf = np.asarray(results)
        min_max_scaler = preprocessing.MinMaxScaler()
